...
__export(src_exports, {
  ...
  NT: () => NT2,
  ProtoHax: () => ProtoHax,
  createArmrCoder: () => createArmrCoder,
  createDeltaCoder: () => createDeltaCoder,
  decodeBitfield: () => decodeBitfield,
  decodeStable: () => decodeStable,
  encodeBitfield: () => encodeBitfield,
  encodeStable: () => encodeStable,
  maybePlayerMove: () => maybePlayerMove,
  tagPlayerMove: () => tagPlayerMove
});
...
// src/pbreflect.ts
var NT = pbjs_pb_default.nested.NT.nested;
...
for (const key of gameActions) {
  M[key] = (data, encoded) => encoded ? NT2.Envelope.encode({ gameAction: { [key]: data } }).finish() : NT2.Envelope.fromObject({ gameAction: { [key]: data } });
}
for (const key of lobbyActions) {
  M[key] = (data, encoded) => encoded ? NT2.Envelope.encode({ lobbyAction: { [key]: data } }).finish() : NT2.Envelope.fromObject({ lobbyAction: { [key]: data } });
}
